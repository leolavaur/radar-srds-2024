...
### Added modifs: Allow GPU memory growth ###
...
class FlwrClient(fl.client.NumPyClient):
    ...
    def __init__(self, cid, with_efc=False, full=False):
        ...
        self.x_train, self.y_train, self.x_test, self.y_test = load_data.load_data(
            self.cid, full=full
        )
        ...
        if with_efc:
            ...
            )  # consider one additional feature (EFC energy)
            ...
            self.x_train = np.append(
                self.x_train,
                y_energies_train.reshape(y_energies_train.shape[0], 1),
                axis=1,
            )
            self.x_test = np.append(
                self.x_test,
                y_energies_test.reshape(y_energies_test.shape[0], 1),
                axis=1,
            )
        else:
        ...
        self.val_data = train_data[
            idx:
        ]  # holdout validation set for threshold calculation
        self.train_data = train_data[
            :idx
        ]  # reduced self.x_train (only benign wo val_data)
        self.attack_data = self.x_train[
            self.y_train == 1
        ]  # the attack data from train set (used only for threshold estimation)
    ...
    def get_parameters(self):
        ...
        )  # get_weights from tensorflow.keras returns the weights as ndarray

    def set_parameters(self, parameters):
        # Server sets model parameters from a list of NumPy ndarrays (Optional)
        self.model.set_weights(
            parameters
        )  # set_weights on local model (similar to get_weights)
    ...
    def evaluate(self, parameters, config):
        ...
        attack_losses = calculate_reconstruction_loss(
            self.attack_data, attack_inference
        )
        ...
        print(
            "\n>> {} Mean Validation Loss (Benign): {} | (Attack): {}".format(
                self.pre_cid, self.threshold_benign, self.threshold_attack
            )
        )
        ...
        # Threshold Criteria #
        ######################
        # test_eval = distance_calc(
        #     losses, self.threshold_benign, self.threshold_attack
        # )  # this considers both benign and attack thresholds
        test_eval = (
            losses > self.threshold_benign
        )  # this considers only the benign threshold (comment out this line to use this criteria)

        acc, rec, prec, f1, mcc, missrate, fallout, auc = eval_learning(
            self.y_test, test_eval
        )
        ...
        print(
            "\n>> Evaluate {} | Threshold {} | Metrics {} | Mean_Attack_Loss {} | Mean_Benign_Loss {}".format(
                self.pre_cid,
                self.threshold_benign,
                output_dict,
                np.mean(losses[self.y_test == 1]),

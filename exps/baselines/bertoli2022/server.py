...
def fit_config(rnd: int):
    config = {"epoch_global": str(rnd), "num_epochs": 1, "batch_size": 32}
    return config
...
def main():
    ...
    if args.with_efc:
        params = model.create_model(
            x_train.shape[1] + 1
        ).get_weights()  # Additional Feature if using EFC
    else:
    ...
    fl.server.start_server("[::]:4687", config={"num_rounds": 10}, strategy=strategy)

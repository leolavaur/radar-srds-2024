...
models = {
    ...
    "EFC": {
        "model": EnergyBasedFlowClassifier(cutoff_quantile=0.95),
        "local_perf": [],
        "cross_perf": [],
    },
}
...
for algo, model in models.items():
    ...
    for silo_name, silo_data in silos.items():
        if algo == "EFC":
            model["model"].fit(
                silo_data["x_train"], silo_data["y_train"], base_class=0
            )  # EFC requires fit(X,y)
            y_test = silo_data["y_test"]
        else:
            if algo == "DeepAE":
                idx = int(
                    silo_data["x_train"][silo_data["y_train"] == 0].shape[0] * 0.9
                )
                ...
                losses = np.mean(
                    np.abs(val_data - model["model"].predict(val_data)), axis=1
                )  # reconstruction MAE on validation set
                threshold = np.quantile(losses, 0.95)  # set an appropriate threshold
                y_test = silo_data["y_test"]
            else:
                model["model"].fit(
                    silo_data["x_train"][silo_data["y_train"] == 0]
                )  # train each model on benign data
                y_test = silo_data["y_test"].map(
                    {0: 1, 1: -1}
                )  # LOF, OneSVM and IF predict methods returns +1 or -1
        ...
        print(
            ">> LOCALIZED >> Algo: {} | Silo: {} | F1-Score: {}".format(
                algo, silo_name, score
            )
        )
    ...
    print(
        ">> Average Local F1-score: {} \u00B1 {}".format(
            np.mean(model["local_perf"]), np.std(model["local_perf"])
        )
    )
    print(
        ">> Average Cross F1-score: {} \u00B1 {}".format(
            np.mean(model["cross_perf"]), np.std(model["cross_perf"])
        )
    )
